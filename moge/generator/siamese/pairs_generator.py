...
class PairsGenerator(DataGenerator):
    ...
    def __getdata__(self, edges_batch):
        ...
        X = {}
        X["input_seq_i"] = self.get_sequence_encodings([self.node_list[node_id] for node_id in X_list[:, 0].tolist()],
                                                       variable_length=False)
        X["input_seq_j"] = self.get_sequence_encodings([self.node_list[node_id] for node_id in X_list[:, 1].tolist()],
                                                       variable_length=False)
    ...
    def load_data(self, return_sequence_data=False, batch_size=None):
        ...
        if return_sequence_data:
            ...
            X_seq["input_seq_i"] = self.get_sequence_encodings(
                [self.node_list[node_id] for node_id in X[:, 0].tolist()],
                variable_length=False)
            X_seq["input_seq_j"] = self.get_sequence_encodings(
                [self.node_list[node_id] for node_id in X[:, 1].tolist()],
                variable_length=False)
