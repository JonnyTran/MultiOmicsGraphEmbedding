...
class RGCNConv(MessagePassing):
    ...
    def forward(self, x, edge_index, edge_type, target_node_type, src_node_type):
        ...
        for i in range(self.num_edge_types):
            ...
            if self.args.Norm4:
                out.add_(
                    F.normalize(self.propagate(edge_index[:, mask], x=x, edge_type=i, src_node_type=src_node_type)))
            else:
...
class RGNN(torch.nn.Module):
    ...
    def reset_parameters(self):
        ...
        Feat_list = [os.path.join(root, i) for i in
                     ['./author_FEAT.npy', './field_of_study_FEAT.npy', './institution_FEAT.npy']]
