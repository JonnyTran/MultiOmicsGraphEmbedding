...
from moge.model.PyG.utils import (
    join_metapaths,
...
class LATTEConv(MessagePassing, RelationAttention):
    def __init__(
        ...
        self.linear_l = nn.ModuleDict(
            {node_type: nn.Linear(input_dim, output_dim, bias=True) for node_type in self.node_types}
        )  # W.shape (F x F)
        ...
        self.rel_attn_l = nn.ParameterDict(
            {ntype: nn.Parameter(Tensor(attn_heads, self.out_channels)) for ntype in self.node_types}
        )
        ...
        self.rel_attn_bias = nn.ParameterDict(
            {
                ...
                for ntype in self.node_types
                if self.num_tail_relations(ntype) > 1
            }
        )
    ...
    return "linear_l={}, linear_r={}, attn={}, bias={}".format(
        self.linear_l, self.linear_r if hasattr(self, "linear_r") else None, self.attn, self.bias is not None
    )
    ...
    if global_node_index[ntype].size(0) == 0 or self.num_tail_relations(ntype) <= 1:
        continue
    ...
    edge_pred_dicts.update(edge_pred_dict)
            ...
            if verbose:
                ...
                for i, (metapath, beta_mean, beta_std) in enumerate(
                        zip(
                            self.get_tail_relations(ntype) + [ntype],
                    ...
                    if metapath in edge_pred_dict:
                        edge_index = (
                            edge_pred_dict[metapath]
                            if isinstance(edge_pred_dict[metapath], Tensor)
                            else edge_pred_dict[metapath][0]
                        )
                    ...
                    print(
                        f"   - {'.'.join(metapath[1::2]) if isinstance(metapath, tuple) else metapath}, "
            ...
            if verbose:
    print(
        f"   -> {self.activation.__name__ if hasattr(self, 'activation') else ''} "
        ...
        if save_betas:
            try:
                self.update_relation_attn(
                    betas={ntype: betas[ntype].mean(-1) for ntype in betas},
                    global_node_index=global_node_index,
                    batch_sizes=batch_sizes if (self.layer + 1) == self.n_layers else None,
                )
                self.update_edge_attn(
                    edge_index_dict=edge_pred_dict,
                    global_node_index=global_node_index,
                    batch_sizes=batch_sizes if (self.layer + 1) == self.n_layers else None,
                )
    ...
    def projection(self, feats: Dict[str, Tensor], linears: ModuleDict, subset=None):
        h_dict = {
            ntype: linears[ntype].forward(x).relu_() for ntype, x in feats.items() if not subset or ntype in subset
        }

        h_dict = {
            ntype: h.view(feats[ntype].size(0), self.attn_heads, self.out_channels) for ntype, h in h_dict.items()
        }
    ...
    def aggregate_relations(
        ...
        emb_relations = torch.zeros(
            size=(sizes[ntype], self.num_tail_relations(ntype), self.attn_heads, self.out_channels)
        ).type_as(self.attn)
        ...
        for metapath in self.get_tail_relations(ntype, order=1):
            ...
            if (
                    metapath not in edge_index_dict
                    or edge_index_dict[metapath] is None
                    or head not in sizes
                    or tail not in sizes
            ):
                continue
    ...
    higher_order_edge_index = join_edge_indexes(
        edge_index_dict_A=edge_pred_dict,
...
class LATTE(nn.Module):
    ...
    h_dict, edge_pred_dict = self.layers[l].forward(
        feats=h_dict,
        edge_index_dict=edge_index_dict,
        global_node_index=global_node_index,
        edge_pred_dict=edge_pred_dict,
    ...
    for ntype in h_dict:
        ...
        if self.layer_pooling is None or self.layer_pooling in ["last"] or self.n_layers == 1:
            ...
            h_dict = {ntype: torch.cat(h_list, dim=1) for ntype, h_list in h_layers.items() if len(h_list)}
        else:
    ...
    if ntype not in current_dst_ntypes:
        continue
                        ...
                        selfloop_link = pd.Series(
                            {
                                ...
                                "target": nid,
                                "label": ntype,
                                "color": nodes.loc[nid, "color"],
                                "mean": selfloop_weight,
                                "std": 0.0,
                                "layer": latte.layer,
                            },
