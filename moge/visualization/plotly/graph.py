...
def process_pos(pos: Union[DataFrame, Dict[str, Dict[str, float]]]) -> Dict[str, Tuple[float, float, float]]:
    if isinstance(pos, DataFrame):
        cols = ["pos1", "pos2", "pos3"] if "pos3" in pos.columns else ["pos1", "pos2"]
        pos_dict = pos[cols].to_dict(orient="index")
        pos_dict = {node: [pos_dict["pos1"], pos_dict["pos2"], pos_dict["pos3"] if "pos3" in pos_dict else None] \
                    for node, pos_dict in pos_dict.items()}

    elif isinstance(pos, dict) and isinstance(pos[list(pos.keys())[0]], dict):
        pos_dict = {node: [pos_dict["pos1"], pos_dict["pos2"], pos_dict["pos3"] if "pos3" in pos_dict else None] \
                    for node, pos_dict in pos.items()}

    elif isinstance(pos, dict) and isinstance(pos[list(pos.keys())[0]], (list, tuple, np.ndarray)):
        pos_dict = pos

    else:
        raise Exception(f"Input is not of right type: {pos}")
