...
from moge.dataset.utils import (
    get_edge_index_values,
    get_edge_index_dict,
    tag_negative_metapath,
...
class HeteroNetwork(AttributedNetwork, TrainTestSplit):
    def __init__(
        self, multiomics: MultiOmics, node_types: List, layers: Dict[Tuple[str], nx.Graph], annotations=True
    ) -> None:
        """
    ...
    def isolated_nodes(self, ntypes: Set[str] = None) -> Dict[str, Set[str]]:
        ...
        """
        isolated_nodes = {
            ntype: set(nodelist) for ntype, nodelist in self.nodes.items() if not ntypes or ntype in ntypes
        }
    ...
    def get_node_degrees(self, undirected=True) -> pd.DataFrame:
        node2ntype = {node: ntype for ntype, nodes in self.nodes.items() for node in nodes}

        degree_dfs = []
        for metapath, g in tqdm.tqdm(self.networks.items()):
            degrees = g.degree()
            degrees = pd.Series({k: v for k, v in degrees if k in node2ntype}, name=".".join(metapath)).to_frame()
    ...
    def process_annotations(self):
        ...
        logger.info(
            "All annotation columns (union): {}".format(
                {col for _, annotations in self.annotations.items() for col in annotations.columns.tolist()}
            )
        )

    def process_feature_tranformer(
            self,
            columns: List[str] = None,
            ntype_subset: List[str] = None,
            delimiter: List[str] = "\||;",
            labels_subset=None,
            min_count=0,
            verbose=False,
        ...
        for ntype in self.node_types:
            if ntype_subset and ntype not in ntype_subset:
                continue
    ...
    self.feature_transformer = self.get_feature_transformers(
        all_annotations, labels_subset=labels_subset, min_count=min_count, delimiter=delimiter, verbose=verbose
    )
    ...
    def add_edges(
            self,
            edgelist: Union[nx.Graph, List[Tuple[str, str, Dict]]],
        ...
        if isinstance(edgelist, Iterable):
            ...
            self.networks[etype].add_edges_from(
                edgelist.edges.data(), source=src_type, target=dst_type, database=database, **kwargs
            )
    ...
    def add_edges_from_ontology(
        ...
        if split_ntype:
            ...
            edge_index_dict.update(
                get_edge_index_dict(graph, nodes=nodes_split, metapaths=metapaths, d_ntype=None, format="nx")
            )

        else:
            edge_index_dict = get_edge_index_dict(
                graph, nodes=nodes, metapaths=select_etypes, d_ntype=d_ntype, format="nx"
            )
    ...
    def add_edges_from_annotations(
            self,
            ontology: GeneOntology,
            filter_dst_nodes: Optional[List[str]],
            src_ntype: str,
            dst_ntype: str,
            train_date: str,
            valid_date: str,
            test_date: str,
            src_node_col="gene_name",
            dst_node_col="go_id",
            split_etype: str = None,
            d_etype="associated",
        ...
        if split_etype:
            ...
            pred_metapaths = [(src_ntype, etype, dst_ntype) for etype in train_ann.reset_index()[split_etype].unique()]
        else:
        ...
        for go_ann in [train_ann, valid_ann, test_ann]:
            ...
            pos_edge_list_dict = get_edge_index_dict(
                pos_graph, nodes=self.nodes, metapaths=metapaths.intersection(pred_metapaths), format="nx"
            )

            for etype, edges in pos_edge_list_dict.items():
                if etype not in pred_metapaths or len(edges) == 0:
                    continue
            ...
            if use_neg_annotations:
                ...
                neg_graph = nx.from_pandas_edgelist(
                    neg_annotations, source=src_node_col, target=neg_dst_node_col, **nx_options
                )
                if split_etype:
                    metapaths = {(src_ntype, etype, dst_ntype) for u, v, etype in neg_graph.edges}
                else:
                    metapaths = set(pred_metapaths)
                neg_edge_list_dict = get_edge_index_dict(
                    neg_graph, nodes=self.nodes, metapaths=metapaths.intersection(pred_metapaths), format="nx"
                )
    ...
    def get_triples(
            self, all_metapaths: List[Tuple[str, str, str]], positive: bool = False, negative: bool = False
        ...
        for metapath in pred_metapaths:
            ...
            if all_metapaths and metapath not in all_metapaths:
                continue
        ...
        triples = {
            key: torch.cat(values, dim=0) if isinstance(values[0], Tensor) else values
            for key, values in triples.items()
        }

        edge_idx = torch.arange(len(triples["head" if positive else "head_neg"]))
        training_idx, validation_idx, testing_idx = (
            edge_idx[torch.cat(training_idx)],
    ...
    def filter_sequence_nodes(self):
        ...
        """
        for ntype in self.node_types:
            if ntype not in self.multiomics.get_omics_list():
                continue
    ...
    def to_dgl_heterograph(
            self,
            node_attr_cols: List[str] = [],
            target="go_id",
            min_count=10,
        ...
        for metapath, etype_graph in tqdm.tqdm(self.networks.items(), desc="Add etype's to DGLHeteroGraph"):
            ...
            if exclude_ntypes and (head_type in exclude_ntypes or tail_type in exclude_ntypes):
                continue
            if exclude_etypes and (
                    metapath in exclude_etypes
                    or etype in exclude_etypes
                    or untag_negative_metapath(metapath) in exclude_etypes
            ):
                continue
        ...
        num_nodes_dict = {
            ntype: n for ntype, n in self.num_nodes_dict.items() if ntype_subset is None or ntype in ntype_subset
        }
        if exclude_ntypes:
    ...
    for col in annotations.columns.drop([target, "omic", SEQUENCE_COL], errors="ignore").intersection(
        node_attr_cols if node_attr_cols is not None else []
    ):
        if col in self.feature_transformer:
            feat_filtered = parse_labels(annotations[col], min_count=None, dropna=False, delimiter=self.delimiter)
    ...
    # Expression values
    if expression and ntype in self.multiomics.get_omics_list() and hasattr(self.multiomics[ntype], "expressions"):
    ...
    if sequence and SEQUENCE_COL in annotations:
        if hasattr(self, "tokenizer"):
            padded_encoding, seq_lens = self.tokenizer.one_hot_encode(ntype, sequences=annotations[SEQUENCE_COL])
    ...
    for mask_name, trainvalidtest_nodes in zip(
        ["train_mask", "valid_mask", "test_mask"],
        [self.train_nodes[ntype], self.valid_nodes[ntype], self.test_nodes[ntype]],
    ):
        if trainvalidtest_nodes is None or len(trainvalidtest_nodes) == 0:
            continue
        nodelist_idx = self.nodes[ntype].get_indexer_for(self.nodes[ntype].intersection(trainvalidtest_nodes))
    ...
    if ntype not in self.annotations or target not in self.annotations[ntype].columns:
        continue
    y_label = parse_labels(
        self.annotations[ntype].loc[self.nodes[ntype], target],
        min_count=None,
        labels_subset=None,
        dropna=False,
    ...
    for mask_name, trainvalidtest in zip(["train_mask", "valid_mask", "test_mask"], [training, validation, testing]):
        for metapath, edge_attr in edge_attr_dict.items():
            src, dst = edge_index_dict[metapath]
            trainvalidtest[metapath] = G.edge_ids(src[edge_attr[mask_name]], dst[edge_attr[mask_name]], etype=metapath)
        elif "node" in train_test_split:
            for mask_name, trainvalidtest in zip(
                    ["train_mask", "valid_mask", "test_mask"], [training, validation, testing]
            ):
                for ntype in G.ntypes:
                    if mask_name not in G.nodes[ntype].data:
                        continue
    ...
    transform_cols = [col for col in transform_cols if col not in self.feature_transformer]
        ...
        for metapath in networks_prog:
            ...
            hetero[metapath].edge_index, edge_attrs = get_edge_index_values(
                ...
                nodes_B=self.nodes[tail_type],
                edge_attrs=["train_mask", "valid_mask", "test_mask"]
                if "edge" in train_test_split or inductive
                else None,
                format="pyg",
            )
        ...
        for ntype in tqdm.tqdm(node_types, desc="Adding node attrs to node types"):
            ...
            for col in annotations.columns.drop([target, "omic", SEQUENCE_COL], errors="ignore").intersection(
                ...
                if col in self.feature_transformer:
                    feat_filtered = parse_labels(
                        annotations[col], min_count=None, dropna=False, delimiter=self.delimiter
                    )
            ...
            if (
                    expression
                    and ntype in self.multiomics.get_omics_list()
                ...
                if hasattr(expressions, "sparse") and not expressions.empty:
                    ...
                    hetero[ntype]["x"] = SparseTensor(
                        rowptr=torch.tensor(csr_mtx.indptr, dtype=torch.long),
                        col=torch.tensor(csr_mtx.indices, dtype=torch.long),
                        value=torch.tensor(csr_mtx.data, dtype=torch.float),
                        sparse_sizes=csr_mtx.shape,
                        is_sorted=True,
                        trust_data=True,
                    )
    ...
    if isinstance(labels, np.ndarray):
        ...
        if hasattr(self, "train_nodes") and self.train_nodes:
    train_nodes_idx = {
        ntype: nodelist.get_indexer_for(nodelist.intersection(self.train_nodes[ntype]))
        for ntype, nodelist in self.nodes.items()
        if self.train_nodes[ntype]
    }
    valid_nodes_idx = {
        ntype: nodelist.get_indexer_for(nodelist.intersection(self.valid_nodes[ntype]))
        for ntype, nodelist in self.nodes.items()
        if self.valid_nodes[ntype]
    }
    test_nodes_idx = {
        ntype: nodelist.get_indexer_for(nodelist.intersection(self.test_nodes[ntype]))
        for ntype, nodelist in self.nodes.items()
        if self.test_nodes[ntype]
    }
