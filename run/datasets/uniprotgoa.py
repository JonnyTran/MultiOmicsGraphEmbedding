...
def load_uniprotgoa(
    ...
    geneontology = UniProtGOA(
        path=os.path.dirname(uniprotgoa_path),
        file_resources={
    ...
    for dst_ntype in set(["biological_process", "molecular_function", "cellular_component"]).difference(
        ...
        network.add_edges_from_annotations(
            geneontology,
            filter_dst_nodes=network.nodes[dst_ntype],
            src_ntype=head_node_type,
            dst_ntype=dst_ntype,
    ...
    if "DeepGraphGO" in name:
        dgg_go_id["go_id"] = dgg_go_id["go_id"].map(lambda d: d if isinstance(d, (list, np.ndarray)) else [])
    ...
    dataset = HeteroNodeClfDataset.from_heteronetwork(
        ...
        if hparams.ntype_subset
        else set(network.nodes.keys()).difference(hparams.pred_ntypes),
    ...
    cls_network = dgl.heterograph(get_edge_index_dict(geneontology.network, nodes=cls_network_nodes, format="dgl"))
